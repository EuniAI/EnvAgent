...
def info(url: str, commit_id: Optional[str]):
    ...
    try:
        repository = repository_service.repository_storage.get_repository_by_url_and_commit_id(url, commit_id)
        ...
        if path.exists():
            click.echo(f"Path Status: ✅ Exists")
            # Show directory size
            total_size = sum(f.stat().st_size for f in path.rglob('*') if f.is_file())
            size_mb = total_size / (1024 * 1024)
            click.echo(f"Directory Size: {size_mb:.2f} MB")
        else:
            click.echo(f"Path Status: ❌ Missing")
        
        # Check if knowledge graph exists in Neo4j
        try:
            kg_exists = repository_service.kg_service.kg_handler.knowledge_graph_exists(repository.kg_root_node_id)
...
def export(format: str):
    ...
    try:
        ...
        if format == 'json':
            ...
            for i, repo in enumerate(repositories, 1):
                commit_short = (repo.commit_id[:12] + "...") if repo.commit_id and len(repo.commit_id) > 15 else (repo.commit_id or "Latest")
                url_short = repo.url[:37] + "..." if len(repo.url) > 40 else repo.url
                path_exists = Path(repo.playground_path).exists()
                status = "✅ OK" if path_exists else "❌ Missing"
                
                click.echo(f"{i:<3} {url_short:<40} {commit_short:<15} {repo.kg_root_node_id:<8} {status:<10}")
