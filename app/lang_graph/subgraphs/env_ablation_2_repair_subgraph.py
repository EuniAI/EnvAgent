...
from app.lang_graph.repair_nodes.env_repair_gen_single_command_node import EnvRepairGenSingleCommandNode
...
class EnvAblation2RepairSubgraph:
    def __init__(
        self,
        debug_mode: bool,
        test_mode: str,
        ...
        def create_router_mapping():
                base_mapping = {
                    "case1": "execute_env",
                    "case2": "gen_single_command_bashfile",  # 使用新节点：只分析错误并生成完整bashfile
                    "success": END,
                }
                ...
                if test_mode == "generation":  # generation 模式下，case3（环境成功，但还没有运行测试）应该执行测试
                    ...
                    base_mapping["case4"] = "gen_single_command_bashfile" # generation 模式下，case4（测试失败）直接生成全局bashfile（已包含测试错误分析）
        ...
        env_repair_execute_node = EnvRepairExecuteNode(container, ablation_mode="2")
        # 这里需要只做分析，然后直接生成全局的bashfile，不做单命令生成和update
        env_repair_gen_single_command_node = EnvRepairGenSingleCommandNode(advanced_model, container)
        ...
        workflow.add_node("gen_single_command_bashfile", env_repair_gen_single_command_node)  # 分析错误并生成完整bashfile
        ...
        workflow.add_edge("gen_single_command_bashfile", "execute_env")
