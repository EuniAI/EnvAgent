...
class FileContextRetrievalSubgraph:
    ...
    def __init__(
        ...
        file_context_provider_tools = ToolNode(
            tools=file_context_provider_node.tools,
        ...
        workflow.add_conditional_edges(
            "file_context_provider_node",
            functools.partial(tools_condition, messages_key="context_provider_messages"),
            {"tools": "file_context_provider_tools", END: "context_extraction_node"},
        )
        workflow.add_edge("file_context_provider_tools", "file_context_provider_node")
        workflow.add_edge("context_extraction_node", "reset_context_provider_messages_node")
        workflow.add_edge("reset_context_provider_messages_node", "file_context_refine_node")

        # If refined_query is non-empty, loop back to provider; else terminate
        workflow.add_conditional_edges(
            "file_context_refine_node",
            lambda state: bool(state["refined_query"]),
            {True: "file_context_provider_node", False: END},
        )
